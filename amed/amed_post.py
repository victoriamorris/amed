...
def amed_post_Excel(ifile=None, month=None):
	amed_post(export_type='Excel', ifile=ifile, month=month)
	
	
def amed_post_LM(ifile=None, month=None):
	amed_post(export_type='LM', ifile=ifile, month=month)
...
def amed_post(export_type='LM', ifile=None, month=None):
    ...
    if month:
        try: month = int(month)
        except: exit_prompt('Error: The value of the parameter --month must be an integer')
